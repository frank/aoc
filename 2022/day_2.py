...
def get_data_two() -> NDArray[int]:
    ...
    with open("inputs/day_2.txt", "r") as file:
        for line in file.readlines():
            strategy.append(
                [strategy_map.get(s, s) for s in line.rstrip("\n").split(" ")]
            )
    strategy = np.array(strategy)

    # convert the strategy list to the same format as the first problem
    strategy[np.where(strategy[:, 1] == "X"), 1] = (
        np.mod(strategy[np.where(strategy[:, 1] == "X"), 0].astype("int32") - 2, 3) + 1
    )
    strategy[np.where(strategy[:, 1] == "Y"), 1] = strategy[
        np.where(strategy[:, 1] == "Y"), 0
    ]
    strategy[np.where(strategy[:, 1] == "Z"), 1] = (
...
def count_scores(strategy: NDArray[int]):
    ...
    print(
        play_score + draw_score + rock_win_score + paper_win_score + scissors_win_score
    )
